...
#[structopt(
    name = "strtod-simple-bench",
    about = "strtod benchmark utility",
    no_version
)]
struct Opt {
...
enum Cmd {
    ...
    /// Run all benchmarks for strtod only
    All {
        /// Number of random floats generated
        #[structopt(short = "n", default_value = "50000")]
        count: usize,
...
fn run_bench<F: Fn(&str) -> f64>(inputs: &[String], repeat: usize, func: F) -> Vec<i64> {
...
impl Method {
    ...
    fn run_as(&self, input: &Input, repeat: usize, name: &str) -> BenchResult {
        let data = &input.data;
        let times = match self {
            Self::Strtod => run_bench(data, repeat, |s: &str| {
                strtod::strtod(s).unwrap_or_default().0
            }),
            Self::FastFloat2 => run_bench(data, repeat, |s: &str| {
                fast_float2::parse_partial::<f64, _>(s)
                    .unwrap_or_default()
                    .0
            }),
            Self::FastFloat => run_bench(data, repeat, |s: &str| {
                fast_float::parse_partial::<f64, _>(s).unwrap_or_default().0
            }),
            Self::Lexical => run_bench(data, repeat, |s: &str| {
                lexical_core::parse_partial::<f64>(s.as_bytes())
                    .unwrap_or_default()
                    .0
            }),
            Self::FromStr => {
                run_bench(data, repeat, |s: &str| s.parse::<f64>().unwrap_or_default())
            }
        };
    ...
    pub fn run(&self, input: &Input, repeat: usize, name: &str) -> BenchResult {
        self.run_as(input, repeat, name)
    }
...
fn main() {
    ...
    let methods = if !opt.only_strtod && !matches!(&opt.command, &Cmd::All { .. }) {
        Method::all().into()
    } else {
        vec![Method::Strtod]
    };
    ...
    for input in &inputs {
        for method in &methods {
            ...
            results.push(method.run(input, opt.repeat.max(1), name));
        }
    }
